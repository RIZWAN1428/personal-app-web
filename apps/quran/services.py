"""
Service helper for Quran API (AlQuran Cloud).
Fetches Surahs, Ayahs, Translations (English & Urdu), Audio Recitations, and Search.
"""
import json
import logging
import urllib.parse
...
# Complete list of 114 Surahs with metadata for instant rendering
SURAH_LIST = [
    ...
    {"number": 5, "name": "المائدة", "englishName": "Al-Maaida", "englishNameTranslation": "The Table", "numberOfAyahs": 120, "revelationType": "Medinan"},
    ...
    {"number": 33, "name": "الأحزاب", "englishName": "Al-Ahzaab", "englishNameTranslation": "The Clans", "numberOfAyahs": 73, "revelationType": "Medinan"},
    ...
    {"number": 37, "name": "الصافات", "englishName": "As-Saaffaat", "englishNameTranslation": "Those drawn up in Ranks", "numberOfAyahs": 182, "revelationType": "Meccan"},
    ...
    {"number": 42, "name": "الشورى", "englishName": "Ash-Shura", "englishNameTranslation": "Consultation", "numberOfAyahs": 53, "revelationType": "Meccan"},
    {"number": 43, "name": "الزخرف", "englishName": "Az-Zukhruf", "englishNameTranslation": "Ornaments of gold", "numberOfAyahs": 89, "revelationType": "Meccan"},
    {"number": 44, "name": "الدخان", "englishName": "Ad-Dukhaan", "englishNameTranslation": "The Smoke", "numberOfAyahs": 59, "revelationType": "Meccan"},
    {"number": 45, "name": "الجاثية", "englishName": "Al-Jaathiya", "englishNameTranslation": "Crouching", "numberOfAyahs": 37, "revelationType": "Meccan"},
    ...
    {"number": 49, "name": "الحجرات", "englishName": "Al-Hujuraat", "englishNameTranslation": "The Inner Apartments", "numberOfAyahs": 18, "revelationType": "Medinan"},
    ...
    {"number": 88, "name": "الغاشية", "englishName": "Al-Ghaashiya", "englishNameTranslation": "The Overwhelming Event", "numberOfAyahs": 26, "revelationType": "Meccan"},
    ...
    {"number": 97, "name": "القدر", "englishName": "Al-Qadr", "englishNameTranslation": "The Power, Fate", "numberOfAyahs": 5, "revelationType": "Meccan"},
    ...
    {"number": 102, "name": "التكاثر", "englishName": "At-Takaathur", "englishNameTranslation": "Competition in increase", "numberOfAyahs": 8, "revelationType": "Meccan"},
...
def get_surah_by_number(number):
    """Finds a surah metadata item by number (1-114)."""
    for s in SURAH_LIST:
...
def get_surah_verses(surah_number):
    """
    Fetches Arabic text (Uthmani), English translation (Saheeh International),
    and Urdu translation (Jalandhry) for a given Surah number.
    """
    url = f"https://api.alquran.cloud/v1/surah/{surah_number}/editions/quran-uthmani,en.sahih,ur.jalandhry"
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "PersonalApp-Quran/1.0"})
        with urllib.request.urlopen(req, timeout=10) as response:
            ...
            if data.get("code") == 200:
                ...
                urdu_data = editions[2] if len(editions) > 2 else {"ayahs": []}
                ...
                for i in range(len(arabic_data["ayahs"])):
                    ar = arabic_data["ayahs"][i]
                    en = english_data["ayahs"][i] if i < len(english_data["ayahs"]) else {}
                    ur = urdu_data["ayahs"][i] if i < len(urdu_data["ayahs"]) else {}

                    ayah_num_in_surah = ar["numberInSurah"]
                    global_num = ar["number"]

                    ayahs.append({
                        "global_number": global_num,
                        "number_in_surah": ayah_num_in_surah,
                        "arabic": ar["text"],
                        "english": en.get("text", ""),
                        "urdu": ur.get("text", ""),
                        "juz": ar.get("juz", 1),
                        "manzil": ar.get("manzil", 1),
                        "page": ar.get("page", 1),
                        "sajda": ar.get("sajda", False),
        ...
        logger.warning(f"Error fetching Surah {surah_number}: {e}")

    surah_meta = get_surah_by_number(surah_number) or {
        "number": surah_number,
        "name": "القرآن الكريم",
    ...
    return {
        ...
        "surah": surah_meta,
        "ayahs": [],
        "error": "Could not connect to Quran API. Please check your internet connection.",
    }


def search_quran(query):
    """Searches Quran for matching verses in English."""
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=10) as response:
            ...
            if data.get("code") == 200:
                ...
                return {"success": True, "count": data["data"]["count"], "results": results}
    except Exception as e:
        logger.warning(f"Error searching Quran for '{query}': {e}")

    return {"success": False, "count": 0, "results": [], "error": "Search failed or no matches found."}
